...
<%@ taglib uri="http://www.ibm.com/xmlns/prod/websphere/portal/v8.0/portal-navigation" prefix="portal-navigation" %>
<%@ taglib uri="http://www.ibm.com/xmlns/prod/websphere/portal/v8.0/resolver" prefix="r" %>
...
<c:if test="${startLevel == curLevel}">
	...
	<div role="navigation" aria-expanded="false" class="wpthemeNavCollapsed ${rootClass} <c:if test="${deviceClass == 'tablet'}"> wpthemeMobileSide</c:if>" id="wpthemeNavRoot">
</c:if>
...
<c:forEach var="node" items="${wp.navigationModel.children[parent]}">
	...
	<c:if test="${hasChildren == false}">
		<c:if test="${startLevel != curLevel}">
		<div<c:if test="${deviceClass == 'tablet'}"> class="wpthemeNavCollapsed ${rootClass} wpthemeMobileSide"</c:if> id="${parentNodeID}_navigationSubnav">
		</c:if>
	...
	<%-- Output a link to nodeID --%>
		<c:set var="isCollapsed" value="${wp.selectionModel[node] == null || selectedNodeID == nodeID || deviceClass == 'tablet'}" />
		...
		<c:forEach var="child" items="${wp.navigationModel.children[node]}">
			<c:if test="${!child.metadata['com.ibm.portal.Hidden']}">
				<c:set var="hasVisibleChildren" value="true"/>
			</c:if>
		</c:forEach>
			<li role="treeitem" class="wpthemeNavListItem<c:if test="${selectedNodeID == nodeID}"> wpthemeSelected</c:if><c:if test="${wp.selectionModel[node] != null && deviceClass == 'tablet'}"> wpthemeAncestor</c:if><c:if test="${isCollapsed}"> wpthemeNavCollapsed</c:if><c:if test="${hasVisibleChildren && startLevel + levelsDisplayed - 1 > curLevel}"> wpthemeHasChildren</c:if>" <c:if test="${!isCollapsed}">aria-expanded="true"</c:if><c:if test="${isCollapsed}">aria-expanded="false"</c:if> id="${nodeID}_navigation">
				<a href="?uri=nm:oid:${nodeID}">
					<span id="${nodeID}_navigationLinkText" lang="${node.title.xmlLocale}" dir="${node.title.direction}"><c:out value="${node.title}"/><c:if test="${selectedNodeID == nodeID}"><span class="wpthemeAccess"><portal-fmt:text key="currently_selected" bundle="nls8.Theme"/></span></c:if></span>
				</a>
	...
	<c:if test="${hasVisibleChildren}">
		...
		<c:if test="${startLevel + levelsDisplayed - 1 > curLevel}">
				...
				<a role="button" class="wpthemeExpandBtn" aria-label="${btnValue}" title="${btnValue}" href="javascript:;" onclick="wptheme.toggleMobileNav('${nodeID}_navigation','wpthemeNavCollapsed','<portal-fmt:text key="theme.expand" bundle="nls.commonUI"/>','<portal-fmt:text key="theme.collapse" bundle="nls.commonUI"/>','wpthemeNavRoot',<c:if test="${deviceClass == 'tablet'}">true</c:if><c:if test="${deviceClass != 'tablet'}">false</c:if>,<c:out value="${(curLevel - startLevel) + 1}"/>); <c:out value="${cancelEvent}"/>" id="${nodeID}_navigationLink">
					&nbsp;<img alt="" src="${wp.themeConfig['resources.modules.ibm.contextRoot']}/themes/html/dynamicSpots/icons/blank.gif"/>
				...
				<c:if test="${!(wp.selectionModel[node] == null || selectedNodeID == nodeID) && deviceClass == 'tablet'}">
